...
- Rush and Breakthrough use the creator workspace's corrected public contract: Team 1 attacks and Team 2 defends. Where the installed polygons form a complete phase-boundary chain, they determine HQ phase order, objective/M-COM membership, HQ areas, insertion spawns, and HQ vehicle ownership by strict containment. There is no nearest-object fallback. Incomplete layouts retain disclosed authored-root-order grouping rather than receiving a fabricated spatial assignment. Zero-height retail boundaries use a clearly marked 100 m Portal/Blockly extrusion because the SDK requires a non-zero `PolygonVolume` height.
