...
- Objective spawns are nested under their capture point with stable names. Wake Island Conquest joins each polygon to its installed `gem_capturepoint` identity and derives A-G from the conquest partition's ascending root order (`2, 3, 4, 5, 6, 23, 24`), including its 11,251.8 m² G capture volume.
