...
- The `Objectives` scene branch is alphabetical. Vehicle pads are separated into `Vehicles/HQ/Team1|Team2`, `Vehicles/Objectives/Team1|Team2|Shared`, and `Vehicles/Emplacements`, including automatic AA guns.
- Aircraft-enabled Conquest layouts promote the largest non-HQ authored boundary to `CombatArea.SurroundingVolume`, preserving the game-authored flight space separately from the inner `CombatVolume`.
