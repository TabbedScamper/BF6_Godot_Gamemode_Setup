...
| `breakthrough` | Attackers take every control point in the current sector, then the frontline advances. Objective labels restart locally as A/B (or only A when a sector has one). | Ordered Sector nodes, each owning one or two CapturePoints; sector-local ObjIds and HQ/vehicle activation. | Partial. Sector nodes and local A/B wiring now exist. The geometric classifier still misses capture areas on some maps and needs sector-boundary mining. |
| `rush` | Attackers destroy M-COM pairs sector by sector; there are no CapturePoints. Vehicles belong to HQ/phase supply, not capture points. | `Objectives/Sectors/SectorN/MCOM-A|B`, ordered `Sector.MCOMs`, no CapturePoint nodes, HQ-linked vehicles. | Corrected. MCOMs are objectives, captures are suppressed, and vehicles can no longer bind to a false capture. Exact order is currently derived from the earliest authored HQ and must be validated against runtime links. |
