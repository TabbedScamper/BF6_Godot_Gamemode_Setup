...
| `breakthrough` | Team 1 attackers must hold every control point in the current sector simultaneously, then the frontline advances against Team 2 defenders. Objective labels restart locally as A/B/C (or the authored subset). | Andy-compatible `Sector0` attacker boundary, active `Sector1..N`, and terminal defender boundary; active sectors own their game-authored CapturePoints and phased HQ pair. | Strong where the installed boundary chain is complete. Exact polygon containment determines sector membership, HQ areas, HQ phase order, insertion spawns, and HQ vehicles without a distance fallback. Ambiguous maps retain a disclosed authored-root-order projection. IDs follow the workspace contract: sectors `100+n`, triggers `600+n`, HQs `300+n`/`400+n`, captures in each phase's `1100/1200/...` band, and vehicles at offset `+50`. |
| `rush` | Team 1 attackers destroy the active sector's M-COMs before their tickets run out; there are no CapturePoints. Vehicles belong to HQ/phase supply, not capture points. | Andy-compatible boundary/active/boundary Sector shell; `MCOM-A|B` under each active sector; staged HQ pairs and HQ-linked vehicles. | Strong where installed sector polygons uniquely contain every M-COM. The workspace's `CountOf(Sectors)-2` and ID arithmetic are reproduced. MCOM IDs remain phase-paired (`201/202`, `203/204`, ...), including reserved gaps when a phase has only one live objective. If containment is incomplete or ambiguous, authored M-COM order is retained and labeled; no distance assignment is used. No CapturePoint is synthesized. |
