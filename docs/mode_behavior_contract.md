...
| `operations` | Sector-based attacker/defender progression related to Breakthrough; official patch data refers to Objective A in Sector 2. | Selected sector/HQ/capture candidate graph; live phase and battalion sequence must be modeled separately. | Exact selected sector-to-HQ and sector-to-capture inspector arrays build on Capstone, Contaminated, Tungsten, and Subsurface, including selected cross-partition objects. Candidate folders follow authored collector order, not a proved live phase order. Three battalions, ticket defaults, and timing are decoded; activation and cross-map persistence remain incomplete. |
...
| `payload` | Escort payloads along authored splines through ordered checkpoints and sectors. | Payload roots, spline/route state, ordered checkpoints, team control, and phase timing; do not infer the route from polygon size. | Six selected Payload controllers now show their exact attached sampled spline as an editor `Path3D`; four selected maps have no proved route link and remain unresolved. The retail interfaces expose transform, velocity, direction, control, distance, and checkpoint state, while Portal exposes no Payload/spline objective API. |
