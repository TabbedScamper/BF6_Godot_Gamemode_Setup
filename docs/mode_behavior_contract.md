...
| `breakthrough` | Attackers must hold every control point in the current sector simultaneously, then the frontline advances. Team 2 occupies the opening/attacker side and Team 1 the forward/defender side. Objective labels restart locally as A/B/C (or the authored subset). | Andy-compatible `Sector0` attacker boundary, active `Sector1..N`, and terminal defender boundary; active sectors own one to three CapturePoints and a phased HQ pair. | Strong. Installed phase polygons determine membership where unambiguous; bounded proximity is the fallback. IDs follow the workspace contract: sectors `100+n`, triggers `600+n`, HQs `300+n`/`400+n`, captures in each phase's `1100/1200/...` band, and vehicles at offset `+50`. Capture polygons and game-authored neutral alternate spawns are linked to each objective's Portal properties. The corrected team direction differs from the supplied older template and is explicitly marked as a compatibility adjustment. If the install has fewer HQ placements than the workspace addresses, the missing API identity is an explicitly marked alias of a same-team game-authored transform/spawn set. |
